...
float canon_range(char* arr[150][150], int scale_factor,float theta)
{
    double x=0, y=0;
    // Starting Velocity
    ...
    while (1)
    {
        ...
        if(x>0 && y>0)
        {
	        int temp_x = x;
	        int temp_y = y;
	        const int index_vertical  = (int)(temp_x/scale_factor);
	        const int index_horizontal  = (int)(temp_y/scale_factor);
	        arr[index_vertical][index_horizontal] = "*";
